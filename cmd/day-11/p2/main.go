...
func main() {
	...
	var common_divisor int64
	for _, monkey := range monkeys {
		if common_divisor == 0 {
			common_divisor = int64(monkey.Test.Divisor())
		} else {
			common_divisor *= int64(monkey.Test.Divisor())
		}
	}
	...
	for i := 0; i < 10000; i++ {
		for monkeyNum, m := range monkeys {
			fmt.Printf("Monkey %d:\n", monkeyNum)
			for item, err := m.Items.Dequeue(); err == nil; item, err = m.Items.Dequeue() {
				fmt.Printf("\tMonkey inspects an item with a worry level of %d.\n", item)
				newItem := m.Operation.Do(item)
				fmt.Printf("\t\tWorry level is changed to %d.\n", newItem)
				newItem %= monkey.Item(common_divisor)
				fmt.Printf("\t\tWorry level relaxes to %d (%% %d).\n", newItem, common_divisor)
				throwTarget := m.Test.Route(newItem)
